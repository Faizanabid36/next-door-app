    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                messageInput.focus();
                // update info in view
                $('.messenger-infoView .info-name').html(data.fetch.name.charAt(0).toUpperCase()+data.fetch.name.slice(1));
                $('.m-header-messaging .user-name').html(data.fetch.name.charAt(0).toUpperCase()+data.fetch.name.slice(1));
                // Star status
                data.favorite > 0
